...
function updateCanvasDimensions() {
  canvas.attr({
    //edited  old value 100
    height: window.innerHeight - 700,
    width: window.innerWidth
  });
  canvasWidth = canvas.width();
  //edited actual no minus
  canvasHeight = canvas.height()+150;
  draw();
}
...
//edited actual value -105
  for (var j = 0; j < g.length; j++) {
    g[j].curPos.x = (canvasWidth / 2 - offset / 2) + g[j].curPos.x;
    g[j].curPos.y = (canvasHeight / 2 - 505) + g[j].curPos.y;
...
// Size the text
//edited old is 1500
document.sizeRatio = window.innerWidth / 4500;
